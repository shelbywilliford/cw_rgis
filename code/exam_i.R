...
# 8. Add a new column ptw = hp / wt
# Use mutate() to add the new column.
# Assign to: mtcars_with_ptw

# 9. Filter for cars with 4 cylinders (cyl) and select mpg and cyl columns (use piping)
# Chain filter() and select() using %>%.
# Assign to: mtcars_cyl4_mpg

# 10. Group by number of gears and summarize minimum & maximum values of mpg
...
# Function min() returns a minimum value in a vector.
# Function max() returns a maximum value in a vector.
...
# Assign to: g_gplot1
# Create a boxplot of weight by group, filling boxes by group id.
